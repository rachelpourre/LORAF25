...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            w_flat = weight.view(-1).to(torch.float32)
            w_q4, w_norm = block_quantize_4bit(w_flat, group_size=self._group_size)

            self.weight_q4.copy_(w_q4)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            w_deq = block_dequantize_4bit(self.weight_q4, self.weight_norm)
            w_deq = w_deq.view(*self._shape)
